...
| Target-aware v2 rollout follow-up | FinDB run [33859408068](https://github.com/FPI-TW/findb/actions/runs/33859408068)已通過same-revision CI、兩張ECR digest、v2 candidate preparation與runner-side exact bundle validation；preflight SSM command `3350eefc-c908-4d96-ad3b-3052c405dab6`在host第一行以response code 2拒絕。修正後的唯讀wrapper probe `0af72a2d-32ff-4142-9416-c71f8c560523`為Success／exit 0、stderr空白並回傳exact marker | `AWS-RunShellScript`以`/bin/sh`執行，原本的`set -o pipefail`尚未進入任何read／write host action即失敗；candidate acceptance、record與activation均未執行。probe證明POSIX wrapper可在同一host交由Bash執行，但仍須在合併後以新的main SHA完成FinDB normal deployment與accepted replay，才可關閉這項follow-up |
