...
def test_western_slot_targets_the_previous_us_weekday() -> None:
    ...
    assert (
        us_feed.scheduled_date(datetime(2026, 7, 31, 0, 59, tzinfo=timezone.utc))
        == datetime(2026, 7, 30, tzinfo=timezone.utc).date()
    )
    assert (
        us_feed.target_date(datetime(2026, 7, 31, 0, 59, tzinfo=timezone.utc)).isoformat()
        == "2026-07-29"
    )
    assert (
        us_feed.target_date(datetime(2026, 7, 31, 1, 0, tzinfo=timezone.utc)).isoformat()
        == "2026-07-30"
    )
    assert (
        us_feed.target_date(datetime(2026, 7, 27, 1, 0, tzinfo=timezone.utc)).isoformat()
        == "2026-07-24"
    )
...
def test_us_trade_date_policy_uses_governed_holidays_and_fails_closed() -> None:
    ...
    assert (
        us_feed.target_date(datetime(2026, 7, 4, 1, 0, tzinfo=timezone.utc)).isoformat()
        == "2026-07-02"
    )
    assert (
        us_feed.target_date(datetime(2028, 7, 6, 1, 0, tzinfo=timezone.utc)).isoformat()
        == "2028-07-05"
    )
    assert (
        us_feed.target_date(datetime(2028, 7, 5, 1, 0, tzinfo=timezone.utc)).isoformat()
        == "2028-07-03"
    )
    assert (
        us_feed.target_date(datetime(2027, 11, 27, 1, 0, tzinfo=timezone.utc)).isoformat()
        == "2027-11-26"
    )
    with pytest.raises(ScheduleError, match="governed market calendar"):
        us_feed.target_date(datetime(2029, 1, 5, 1, 0, tzinfo=timezone.utc))
    with pytest.raises(ScheduleError, match="governed market calendar"):
        us_feed.target_date(datetime(2026, 1, 2, 1, 0, tzinfo=timezone.utc))
