...
| Crypto mapping | `crypto.py` | Ticker/symbol normalization for crypto |
| Equity and index mapping | `equity.py` + `usstock.py` | US/global stock + regional index handling |
| Futures mapping | `futures.py` | Contract + continuous futures logic |
| Macro mapping | `macro.py` | Series + observation normalization |
